...
for episode in range(n_episodes):
    ...
    iterations_per_episode = 0
    while not done:
        action = env.action_space.sample()  # Sample random action. Replace this with your RL agent
        state, reward, done, info = env.step(action)  # Execute the action
        if done:  # If the episode is finished, print the total reward
            print(f'Episode: {episode + 1}, Final Reward: {reward}, Iterations: {iterations_per_episode}')
